...
function ShowExam() {
    ...
    // },[selectedStudent])
    // useEffect(()=>{
    //     console.log('examStudentsData :: ',examStudentsData)
    // },[examStudentsData])
    useEffect(() => {
        if (examStudentsData) {
            ...
            const membersData = members.map((member, index) => {
                const scoreData = scores.find((item) => item.course_selected_id == member.course_selected) || {};
                return {
                    name: member.name,
    ...
    const handleChange = async (e) => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                uploadedFiles.push(res.data.files);
            } catch (error) {
                ...
                <td>
                    <div className="d-flex justify-content-around">
                        ...
                        <CButton onClick={() => handleModalOpen(item)}>
                            {item.scoreStatus === 1 ? (
                                <NavLink
                                    to={`/admin/UpdateScore/${item.score_id}`}
                                    style={{
                                        textDecoration: "none",
                                        color: "blue",
                                    }}
    ...
    const downloadImagesAsZip = async () => {
        ...
        for (const user of mergData) {
            ...
            const imagePromises = user.files.map(async (file) => {
                // const url = `https://darolhoda.ir/upload/${file.path}`;
                                        ...
                                        {images.map((image, index) => (
                                            <div key={index}>
                                                <img
                                                    ...
                                                    // src={`https://darolhoda.ir/upload/${image}`}
